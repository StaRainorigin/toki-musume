...
4. **双模式切换** — 监督/陪伴模式决定偷瞄姬的行为强度，同一性格在不同模式下表现不同
5. **SQLite 单文件** — 零运维，方便备份和迁移
6. **YAML 配置** — 用户可读可编辑，不依赖 UI 即可调参

## 预留扩展

- **SetWinEventHook 事件驱动** — 被动监听窗口标题变化、前台切换等事件，替代部分轮询，实现零延迟响应（如游戏结算即时触发）
- **Live2D 集成** — 偷瞄姬的视觉形象，表情随模式/互动变化
